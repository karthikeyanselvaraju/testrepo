...
public class YCSItemDetailAsyncRetrieverTest {

	static {
        System.setProperty("config.file", "conf/mylist-local.conf");
    }
    private final static Logger LOGGER = LoggerFactory.getLogger(YCSItemDetailAsyncRetrieverTest.class);

    private static Application fakeApp;
    private YCSItemDetailAsyncRetriever retriever;
    @Mock
	private ClubPriceDAO dao;
    ...
    public void setup() throws CacheAccessException, OfferServiceException {
    	...
    	retriever = new YCSItemDetailAsyncRetriever(dao);
    ...
    public void testgetAsyncDetailsForSuccess() throws Exception {
    	...
    	// CC offers
        List<Long> storeIds = Arrays.asList(101L,102L, 103L, 104L);
    	Map<Long, ClubPrice> allocatedOffersPostalOnly = getClubPriceOffers(storeIds);
    	when(dao.findItemPrices(any(String.class),any(Integer.class))).thenReturn(allocatedOffersPostalOnly);
    	    	
    	Promise<Map<String, ClubPrice>> result = retriever.getAsyncDetails(itemType, itemMap, vo);
    ...
    public void testgetAsyncDetailsWithContextForSuccess() throws Exception {
    	...
    	Promise<Map<String, ClubPrice>> result = retriever.getAsyncDetails(itemType, itemMap, vo,exeCtx);
    	assertNotNull(result);
    }
    ...
    private Map<Long, ClubPrice> getClubPriceOffers(final List<Long> storeIds) {

        Map<Long, ClubPrice> ccOfferPrices = new HashMap<Long, ClubPrice>();

        for(Long storeId: storeIds) {
        	ClubPrice item = new ClubPrice();
            item.setStoreId(Integer.valueOf(storeId.intValue()));
            ccOfferPrices.put(storeId, item);
        }
