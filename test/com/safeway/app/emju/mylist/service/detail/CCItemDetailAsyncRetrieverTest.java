...
public class CCItemDetailAsyncRetrieverTest {

	static {
        System.setProperty("config.file", "conf/mylist-local.conf");
    }
    private final static Logger LOGGER = LoggerFactory.getLogger(CCItemDetailAsyncRetrieverTest.class);

    private static Application fakeApp;
    private CCItemDetailAsyncRetriever retriever;
    @Mock
    ...
    public void setup() throws CacheAccessException, OfferServiceException {
    	...
    	retriever = new CCItemDetailAsyncRetriever(offerCache, ccAllocationDAO);
    ...
    public void testgetAsyncDetailsForSuccess() throws Exception {
    	...
    	// CC offers
        /*List<Long> ccOfferIds = Arrays.asList(101L,102L, 103L, 104L);
    	Map<Long, CCAllocatedOffer> allocatedOffersPostalOnly = getCCOffers(ccOfferIds);
    	when(ccAllocationDAO.findCCAllocation(any(Integer.class))).thenReturn(allocatedOffersPostalOnly);*/
    	
    	List<Long> ccOfferWithStore = Arrays.asList(201L,202L, 203L, 204L);
    	Map<Long, CCAllocatedOffer> allocatedOffersStoreOnly = getCCOffers(ccOfferWithStore);
    	when(ccAllocationDAO.findCCAllocation(any(Integer.class))).thenReturn(allocatedOffersStoreOnly);
    ...
    private Map<Long, CCAllocatedOffer> getCCOffers(final List<Long> ccOfferIds) {
        ...
        Map<Long, CCAllocatedOffer> ccOfferPrices = new HashMap<Long, CCAllocatedOffer>();
